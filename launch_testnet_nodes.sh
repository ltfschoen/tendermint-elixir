...
new_tab "node_0" \
        "echo 'Loading node_0...'" \
        "bash -c 'echo node_0; tendermint node --home "$TESTNET_ROOT_DIR/$TESTNET_FOLDER/$NODE_0_NAME" --rpc.laddr="$AA:46657" --p2p.laddr="$AA:46656" --p2p.seeds=$SEEDS; exec $SHELL'"

new_tab "node_1" \
        "echo 'Loading node_1...'" \
        "bash -c 'echo node_1; tendermint node --home "$TESTNET_ROOT_DIR/$TESTNET_FOLDER/$NODE_1_NAME" --rpc.laddr="$AA:46667" --p2p.laddr="$AA:46666" --p2p.seeds=$SEEDS; exec $SHELL'"

new_tab "node_2" \
        "echo 'Loading node_2...'" \
        "bash -c 'echo node_2; tendermint node --home "$TESTNET_ROOT_DIR/$TESTNET_FOLDER/$NODE_2_NAME" --rpc.laddr="$AA:46677" --p2p.laddr="$AA:46676" --p2p.seeds=$SEEDS; exec $SHELL'"

new_tab "node_3" \
        "echo 'Loading node_3...'" \
        "bash -c 'echo node_3; tendermint node --home "$TESTNET_ROOT_DIR/$TESTNET_FOLDER/$NODE_3_NAME" --rpc.laddr="$AA:46687" --p2p.laddr="$AA:46686" --p2p.seeds=$SEEDS; exec $SHELL'"
